...
| File size | Python shyaml | yq (mikefarah) | shyaml-rs | vs Python | vs yq |
|-----------+---------------+----------------+-----------+-----------+-------|
| 7KB       | 92 ms         | 4.4 ms         | 2.2 ms    | ~42x      | ~2x   |
| 70KB      | 108 ms        | 12 ms          | 8 ms      | ~14x      | ~1.5x |

shyaml-rs is faster than yq for simple queries due to libfyaml's
efficient parsing and shyaml's minimal feature set (no expression
language to parse). The musl static build adds ~4-20% overhead vs
glibc, depending on file size.
...
This works with all actions, not just ~get-value~:

#+begin_src docshtest
$ echo "a:" | shyaml -q get-values-0 "services"; echo "errlvl: $?"
errlvl: 1
$ echo "a:" | shyaml -q keys "services"; echo "errlvl: $?"
errlvl: 1
$ echo "a:" | shyaml -q values "services"; echo "errlvl: $?"
errlvl: 1
#+end_src
...
**** Preserving types with ~-y~

In YAML, ~true~ is a boolean while ~'true'~ is a string. Without ~-y~,
both would output as ~true~, losing the type distinction:
...
With ~-y~, the output preserves the original type so it can be correctly
re-parsed:
...
**** Null-terminated YAML output (~-0~ with ~-y~)

The ~-0~ (null-terminated) variants can be combined with ~-y~ to get
strict YAML output for each field while keeping ~\0~ as the separator.
This is useful when shell scripts need to re-feed individual values
into ~shyaml~ for further processing, while preserving type information.

The output format is the same as without ~-y~: each field is followed
directly by a ~\0~ byte, with no extra characters between the value and
the separator.

For scalars, ~-y~ preserves type-aware quoting:

#+begin_src docshtest
$ cat <<EOF | shyaml keys-0 -y | tr '\0' '|'
a: 1
b: hello
EOF
a|b|
...
#+begin_src docshtest
$ cat <<EOF | shyaml values-0 -y | tr '\0' '|'
a: 1
b: hello
EOF
1|hello|
...
#+begin_src docshtest
$ cat <<EOF | shyaml key-values-0 -y | tr '\0' '|'
a: 1
b: hello
EOF
a|1|b|hello|
...
This works the same with ~get-values-0 -y~ on sequences:

#+begin_src docshtest
$ cat <<EOF | shyaml get-values-0 -y | tr '\0' '|'
- first
- second
EOF
first|second|
...
And on structs (which outputs key-value pairs):
...
Type-distinguishing quotes are preserved, so a string ~'true'~ stays
quoted while a boolean ~true~ does not:

#+begin_src docshtest
$ echo "a: 'true'" | shyaml values-0 -y | tr '\0' '|'
'true'|
#+end_src

Complex values (sequences, mappings) naturally contain newlines in their
YAML representation:
...
When a base value is ~null~ (empty), the overlay value replaces it entirely.
This allows overlays to populate initially empty sections:
...
In this example, ~config:~ in the base has a null value, so the overlay's
~config~ mapping completely replaces it.
...
When using ~set-value~, ~del~, or ~apply~, shyaml preserves the original
document's formatting, including comments and style choices (flow vs block):
...
**** Flow Style Limitation

Flow-style collections (~[...]~ and ~{...}~) remain in flow style when
modified, but are reformatted to multi-line output. This is a limitation
of the underlying libfyaml library, which preserves the flow-vs-block
distinction but not the one-line compactness of flow collections:
...
If you need compact output, consider using block style in your source
documents, which is preserved correctly:
...
By default, simple scalar values are recognized as their natural YAML
type: numbers stay as numbers, booleans as booleans, and plain text
as strings:
...
#+begin_src docshtest
$ echo "config:" | shyaml set-value config.port 8080
config:
  port: 8080
#+end_src
...
**** Scalar type recognition

Without ~-y~, scalar values are recognized as their natural YAML type.
Numbers, booleans, and null are stored as their respective types, not
as strings:

#+begin_src docshtest
$ echo "x:" | shyaml set-value x.port 8080 \; set-value x.debug true \; set-value x.name hello
x:
  port: 8080
  debug: true
  name: hello
...
However, YAML structures (sequences and mappings) are *not* parsed
without ~-y~ — they are treated as literal strings:

#+begin_src docshtest
$ echo "a:" | shyaml set-value a.items "[1, 2, 3]"
a:
  items: "[1, 2, 3]"
#+end_src

Use ~-y~ when you need to set complex structures (see below).

**** Interpreting Value as YAML

Use ~-y~ to interpret the value as full YAML, including structures
like sequences and mappings:
...
Multiple commands can be chained together using the ~\;~ separator.
Commands are executed sequentially, with each command operating on the
result of the previous one. Only the final result is output.
...
This is more efficient than piping multiple shyaml calls, as the YAML
is parsed only once.
...
**** Reading After Mutations

Chain mutations with read actions to transform then extract, keep type
information:

#+begin_src docshtest
$ echo "a: 'true'" | shyaml get-value \; values-0 -y | tr '\0' '|'
'true'|
#+end_src
...
**** With Multi-document YAML

Compound commands work with multi-document YAML streams. The command
chain is applied to each document independently:
